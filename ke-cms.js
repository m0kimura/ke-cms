...
module.export = class Cms extends Utility {
  constructor(op) {
    this.setting(op);
  }
//
  setting(op) {
    let me=this;
    op=op||{}; this.info(); for(let k in op){this.CFG[k]=op[k];}
...
//          for(i in b){v=me.unstring(b[i]); me.INFOJ[v[0]]=v[1];}
          ...
          let data=me.INFOJ.menu_data||'menu';
...
//
  getInfoj(base) {
    let me=this; let a, f, k, v, i, j, d, r, l; let fn="index.cfg", path=me.SS.PATH;
...
//
  devFrame($) {
    let me=this;
    let data, fname, frame, dname, txt, base;
...
//
  pankuzu() {
    let me=this; let out, u, mem, base, mark, dt=[], i, j, k;
...
//
  sidemenu() {
    ...
    let data=me.INFOJ.sidemenu_data||'menu';
    let grp=me.INFOJ.Group;
...
//
  foot() {
    ...
    let data=me.INFOJ.foot_data||'menu';
    let grp=me.INFOJ.Group;
...
//
  guide() {
    ...
    let data=me.INFOJ.guide_data||'menu';
...
//
  menu() {
    ...
    let data=me.INFOJ.navbar_data||'menu';
...
//
  history() {
    let me=this;
    let data, fname, frame, dname, txt, base, ix, dt, jx, date;
...
//
  menuBuild(op, force) {
    ...
    let dt, fn, el, out, b, d, f, i, j, k, l, m, n, r, s, p, u, v, w, x, t;
...
//    let fd=Fs.openSync(local+'menu.json', "w");
